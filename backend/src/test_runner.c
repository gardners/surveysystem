...
int run_test(char *dir, char *test_file)
{
  ...
  do {
    ...
    while(line[0]) {
      ...
      if (sscanf(line,"definesurvey %[^\r\n]",surveyname)==1) {
      ...
      else if (!strcmp(line,"verify_open_session")) {
        if (validate_session_id(last_sessionid)) {
          tdelta=gettime_us()-start_time; tdelta/=1000;
          fprintf(log,"T+%4.3fms : FATAL : No session ID has been captured. Use extract_sessionid following request directive.\n",tdelta);
          goto fatal;
        ...
        // Build path to session file
        char lockfile[8192];
        snprintf(lockfile, 1024, "%s/sessions/%c%c%c%c/lock",
                 test_dir,
                 last_sessionid[0], last_sessionid[1], last_sessionid[2], last_sessionid[3]);
        fprintf(log, "..veryfing lock file does NOT exists: %s.\n", lockfile);
        if(access(lockfile, F_OK) != -1) {
            fprintf(log, "T+%4.3fms : FAIL : lock file for was session created: %s.\n", tdelta, lockfile);
            goto fail;
        }

      }
      else if (!strcmp(line,"verify_locked_session")) {
        if (validate_session_id(last_sessionid)) {
          tdelta=gettime_us()-start_time; tdelta/=1000;
          fprintf(log,"T+%4.3fms : FATAL : No session ID has been captured. Use extract_sessionid following request directive.\n",tdelta);
          goto fatal;
        ...
        snprintf(lockfile, 1024, "%s/sessions/.%c%c%c%c/lock",
                 test_dir,
                 last_sessionid[0], last_sessionid[1], last_sessionid[2], last_sessionid[3]);
        fprintf(log, "..looking for lock file: %s.\n", lockfile);
        if(access(lockfile, F_OK) == -1) {
            fprintf(log, "T+%4.3fms : FAIL : No lock file for session created, looked for %s.\n", tdelta, lockfile);
            goto fail;
        }
...
int stop_lighttpd(int verbose)
{
  ...
  if (verbose) {
    fprintf(stderr,"killing pid %d for port %d...\n", pid, HTTP_PORT);
  }
